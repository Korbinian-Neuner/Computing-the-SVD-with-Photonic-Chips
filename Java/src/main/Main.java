...
public class Main {
	...
	public static void timeCheckGRK(int count, int minSize, int maxSize) {
		...
		for(int i = minSize; i < maxSize; i++) {
			...
			for(int j = 0; j < count; j++) {
				...
				Matrix[] psvd = GRKSVD.calcSVD(test, (int) 60);
	...
	public static void timeCheckQR(int count, int minSize, int maxSize) {
		...
		for(int i = minSize; i < maxSize; i++) {
			...
			for(int j = 0; j < count; j++) {
				...
				long time = System.currentTimeMillis();
				Matrix[] psvd = QRSVD.calcSVD(test, (int) 14f * i);
				time = System.currentTimeMillis() - time;
	...
	public static void test() {
		Matrix test = Matrix.createRandomMatrix(15, 8, 192365235);
		System.out.println("Test matrix to compute the svd of");
		System.out.println(test);
		
		//Uncomment Algorithm to test
		
		//Matrix[] svd = QRSVD.calcSVD(test, 1000); // QR-GRKSVD digital 
		Matrix[] svd = PhotonicQRSVD.calcSVD(test, 1000); // QR-GRKSVD hybrid
		//Matrix[] svd = GRKSVD.calcSVD(test, 1000); // GRK-GRKSVD digital
		//Matrix[] svd = PhotonicGRKSVD.calcSVD(test, 1000); // GRK-GRKSVD hybrid
		
		
		System.out.println("Diagonal matrix in the middle - should be at least close to diagonal");
		System.out.println(svd[1]);
		Matrix reconstructed = Matrix.multiply(svd[0], Matrix.multiply(svd[1], svd[2]));
		...
		for(int m = s1; m < s1 + 10; m++) {
			for(int n = s2; n < s2 + 10; n++) {
				for(int c = s3; c < s3 + 8; c++) {
					...
					//Matrix[] svd = QRSVD.calcSVD(test, c); // QR-GRKSVD digital    --takes around 80s on default settings probably due to logging
					Matrix[] svd = PhotonicQRSVD.calcSVD(test, c); // QR-GRKSVD hybrid
					//Matrix[] svd = GRKSVD.calcSVD(test, c); // GRK-GRKSVD digital
					//Matrix[] svd = PhotonicGRKSVD.calcSVD(test, c); // GRK-GRKSVD hybrid
